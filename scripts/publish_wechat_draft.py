...
def resolve_wechat_client(config: dict) -> WeChatOfficialAccountClient:
    access_token = os.getenv(config.get("access_token_env", "WECHAT_ACCESS_TOKEN")) if config.get("access_token_env") else None
    app_id = os.getenv(config.get("app_id_env", "WECHAT_APP_ID")) if config.get("app_id_env") else None
    app_secret = os.getenv(config.get("app_secret_env", "WECHAT_APP_SECRET")) if config.get("app_secret_env") else None
    return WeChatOfficialAccountClient(base_url=config.get("base_url", "https://api.weixin.qq.com"), access_token=access_token, app_id=app_id, app_secret=app_secret)
